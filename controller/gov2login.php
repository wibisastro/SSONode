...
switch ($cmd) {
    ...
    default:
        switch ($_GET["cmd"]) {
            case "login":
                header("Location: ".SSONODE."/slogin.php?cmd=request&client=".$_SERVER["SERVER_NAME"]);
            break;
            case "logout":
                //session_destroy(); #---khusus untuk account gak perlu ini
                header("Location: ".SSONODE."/slogout.php?client=".$_SERVER["SERVER_NAME"]);
                exit;
            break;
            case "authorize":
                if ($_GET['token']) {
                    $authorized=file_get_contents(SSONODE."/slogin.php?cmd=authorize&token=".$_GET['token']);
