...
# ---- ver 3.0, 15-Apr-14, downgrade untuk sidalih kpu
...
# ---- ver 4.5, 26-Mei-15, tambahkan fungsi form
# ---- ver 5.3, 28-Mar-16, tambahkan fungsi formatBytes
# ---- ver 6.0, 08-Jan-17, ubah hash untuk e-planning bappenas
*/

class document extends db_connection{
	function document() {
		global $PHP_SELF,$ses;
		$this->bodytype="left";
		$this->forbiden="Script ini bukan untuk dieksekusi langsung";
		$this->hash="GantiDenganHashAndaSendiriJanganPakaiYangIni";
		...
		if ($gov2->error || $this->error) {
			if (!$this->error) {
				if ($gov2->error=="NotLogin") {
                    $_SESSION['landingpage']=$_SERVER['SCRIPT_NAME'];
                 //   unset($this->content);
